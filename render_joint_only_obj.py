...
if __name__ == "__main__":
    ...
    parser.add_argument("--merge_objects", action="store_true", default=True, help="When rendering, render merged objects only (default: True)")
    ...
    out_root = getattr(args, "output_root", None) or os.path.join(dataset.model_path, "render_obj_only")
    ...
    if args.merge_objects:
        ...
        if obj_gaussians:
            ...
            if included_objs:
                merged_all = merge_gaussians(included_objs)
                # replace splits' gaussian model with merged objects only
                splits = [(name, cams, merged_all) for (name, cams, _) in splits]
